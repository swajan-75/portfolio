  ...
  useEffect(() => {
    ...
    const initAudio = async () => {
      try {
        ...
        const response = await fetch("/sound/sound.ogg");
        if (!response.ok) {
...
const KeystrokePreview = () => {
  ...
  // Auto-hide the key after 3 seconds of inactivity
  useEffect(() => {
    if (displayKey) {
      ...
      // Reset the timer whenever a new key is pressed or released
      return () => clearTimeout(timer);
    }
  }, [displayKey, pressedKeys]);
  ...
  return (
    <KeyboardProvider enableSound={enableSound} containerRef={containerRef}>
      <div
        ...
        className={cn(
          "mx-auto w-fit",
          showKeypad && "[zoom:0.8] sm:[zoom:1.25] md:[zoom:1.5] lg:[zoom:1.75] xl:[zoom:2]",
          className,
        )}
      >
