  ...
  return (
    <div className="p-6 bg-neutral-900/50 border border-white/[0.07] rounded-2xl space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {fields.map(({ key, label, placeholder, span, textarea }) => (
          <div key={key} className={span ? "sm:col-span-2" : ""}>
            ...
            {textarea ? (
              <textarea
                rows={3} placeholder={placeholder} value={form[key]}
      ...
      <div className="flex justify-end gap-3 pt-1">
        <button onClick={onCancel} className="px-5 py-2 text-sm text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl transition-colors">
          Cancel
        </button>
        <button
